...
import { AxisEnum, CacheModel, CartesianElementModel, CartesianElementTypeEnum, CoordinatesModel, DimensionModel, DotModel, LineModel, PointerPositionModel, RangeModel, RectModel } from '../../models';
import { LegendComponent } from '../legend/legend.component';
import { CommonModule } from '@angular/common';
import { PointerPositionDirective } from '../../directives';
import { line, curveCardinal } from 'd3-shape';

@Component({
  selector: 'lib-cartesian-chart',
  templateUrl: './cartesian-chart.component.html',
  styleUrl: './cartesian-chart.component.less',
  imports: [
    CommonModule,
    LegendComponent,
...
export class CartesianChartComponent implements OnInit, OnChanges {
  @Input() labelNoData = 'No data available';
  @Input() labelLoading = 'Loading...';
  @Input() isLoading = false;
  @Input() fontFamily = 'Arial';
  @Input() fontSizePx = 12;
  @Input() canvasWidthPx = 500;
  @Input() canvasHeightPx = 500;
  @Input() cartesianValues: CartesianElementModel[] = [];
  @Input() gridColor = '#EEEEEE';
  @Input() gridWidth = 1;
  @Input() axesColor = '#212121';
  @Input() axesWidth = 1;
  @Input() crosshairColor = '#BDBDBD';
  @Input() crosshairWidth = 1;
  @Input() xAxisPointsCount = 10;
  @Input() yAxisPointsCount = 10;
  @Input() isCrosshairEnabled = false;
  @Input() yTopPaddingPercentage = 0;
  @Input() yBottomPaddingPercentage = 0;
  @Input() xAxisLabelPrecision = 2;
  @Input() yAxisLabelPrecision = 2;
  @Input() xCrosshairLabelPrecision = 2;
  @Input() yCrosshairLabelPrecision = 2;
  @Input() pointRadius = 4;
  @Input() highlightZeroLine = false;
  @Input() zeroLineWidth = 2;
  @Input() zeroLineColor = '#212121';
  @Input() zeroLineOpacity = 1;
  @Input() zeroLineDash: [number, number] = [2, 5];
  @Input() topMarginPx = 19.5;
  @Input() leftMarginPx = 59.5;
  @Input() rightMarginPx = 59.5;
  @Input() bottomMarginPx = 39.5;

  @ViewChild('overlays', { static: true })
  public overlays?: ElementRef<HTMLCanvasElement>;

  @ViewChild('canvas', { static: true })
  public canvas?: ElementRef<HTMLCanvasElement>;

  private ctx?: CanvasRenderingContext2D | null;
  public cache?: CacheModel;
  ...
  private getPointsRange(values: CartesianElementModel[], axis: AxisEnum): RangeModel | null {
    ...
    const flattened: CoordinatesModel[] = validValues.map(value => value.points).flat();
  ...
  private getCardinalSplineLines(points: CoordinatesModel[]): LineModel[] {
    const lineGenerator = line<CoordinatesModel>()
      .x(d => d.x)
      .y(d => d.y)
    ...
    const numSamples = Math.floor(totalLength / 5);
    const sampledPoints: CoordinatesModel[] = [];
    ...
    for (let i = 1; i < sampledPoints.length; i++) {
      lines.push({
        startPoint: sampledPoints[i - 1],
        endPoint: sampledPoints[i]
      });
    }
